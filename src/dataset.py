...
def test_hf_push():
    test_data = [{"conversations": [{"from": "system", "value": "Test"}]}]
    test_ds = Dataset.from_list(test_data)
    print("Attempting to push dummy dataset to Hugging Face")
    try:
        test_ds.push_to_hub("TBBTLovers/test-dataset-check", token=os.getenv("HUGGINGFACE_TOKEN"))
        print("✅ Hugging Face push works!")
    except Exception as e:
        print("❌ Hugging Face push failed. Check token or permissions.")
        print(e)
        exit(1)
...
def main():
    ...
    cleaned_dataset.push_to_hub(
        "TBBTLovers/rick-and-morty-transcripts-sharegpt",
